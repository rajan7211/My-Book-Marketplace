  ...
  return (
    <SellerLayout>
      ...
      <motion.section
        ...
        className="mb-6 overflow-hidden rounded-2xl bg-brand-dark text-white shadow-sm"
      >
        <div className="relative p-6 sm:p-7">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                to="/seller/listings"
                className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand-yellow px-4 text-sm font-bold text-brand-dark transition-all hover:bg-brand-yellow-dark hover:scale-105 active:scale-95"
              >
                <FiPlusCircle size={16} /> Create Listing
              </Link>
              <Link
                to="/seller/inventory"
                className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-white/15 bg-white/10 px-4 text-sm font-semibold text-white transition-all hover:bg-white/15 hover:scale-105 active:scale-95"
              >
        ...
        {stats.map(({ label, value, helper, icon: Icon, color, to }) => (
          <motion.div key={label} variants={itemVariants}>
            <Link to={to} className="group block h-full">
              <Card className="h-full transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-lg will-change-transform">
                <CardContent className="flex items-start justify-between gap-4 p-5">
                  ...
                  <FiArrowRight
                    size={16}
                    className="mt-1 text-gray-300 transition-all duration-300 group-hover:translate-x-1 group-hover:text-brand-yellow-dark"
                  />
                </CardContent>
              </Card>
            </Link>
      ...
      <motion.section
        ...
        initial="hidden"
        animate="visible"
        className="mt-8 grid gap-6 xl:grid-cols-[1fr_1.2fr]"
      >
        <motion.div variants={itemVariants}>
          ...
          <Card className="h-full">
            <CardContent className="p-6">
              <div className="mb-5 flex items-center justify-between">
                ...
                <Link
                  to="/seller/orders"
                  className="text-xs font-bold text-brand-yellow-dark transition-all hover:underline hover:scale-105"
                >
                  View all orders →
                </Link>
              </div>
                    ...
                    {(Object.keys(ORDER_STATUS_META) as OrderStatus[]).map((status) => {
                      ...
                      return (
                        <motion.div
                          ...
                          whileTap={{ scale: 0.95 }}
                          className="rounded-xl border border-gray-100 bg-gray-50 p-3 text-center transition-shadow hover:shadow-md cursor-pointer"
                        >
                          <span
                            className={cn(
                              "mx-auto grid h-9 w-9 place-items-center rounded-full transition-transform",
                              meta.color
                            )}
                          >
                            <Icon size={16} />
              ...
              <AnimatePresence mode="wait">
                {loadingListings ? (
                  ...
                  <motion.div
                    ...
                    className="space-y-3"
                  >
                    {[...outOfStock, ...lowStock].slice(0, 6).map((listing, index) => (
                      <motion.div
                        key={listing.id}
                        ...
                        whileHover={{ x: 4 }}
                        className="flex items-center justify-between gap-3 rounded-xl border border-gray-100 bg-white p-3 transition-shadow hover:shadow-md"
                      >
                        <div className="flex min-w-0 items-center gap-3">
                          <img
                            src={listing.book?.coverImage}
                            alt={listing.book?.title ?? "Book"}
                            className="h-12 w-9 shrink-0 rounded-md object-cover ring-1 ring-black/5"
                          />
              ...
              <AnimatePresence mode="wait">
                {loadingOrders ? (
                  ...
                  <motion.div
                    ...
                    className="space-y-3"
                  >
                    {sellerOrders.slice(0, 6).map((order, index) => (
                      <motion.div
                        key={order.id}
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                  ...
                  >
                    {topBooks.map((book, index) => (
                      <motion.div
                        ...
                        className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3 transition-all hover:bg-gray-100 hover:shadow-sm"
                      >
                        <div className="flex min-w-0 items-center gap-3">
          ...
          <Card className="h-full">
            <CardContent className="p-6">
              <div className="mb-4 flex items-center gap-3">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-green-100 text-green-700">
              ...
              <div className="space-y-3 text-sm">
                ...
                <SummaryLine 
                  ...
                  value={formatPrice(
                    sellerOrders.length 
                      ? revenue / sellerOrders.filter((o) => o.status !== "CANCELLED").length || 0 
                      : 0
                  )} 
                />
...
function SellerSectionCard({
  ...
  return (
    <Card className="h-full transition-all duration-300 hover:shadow-xl group will-change-transform">
      <CardContent className="flex h-full flex-col p-6">
        <div className="mb-4 flex items-start justify-between">
          <motion.span
            whileHover={{ rotate: 5, scale: 1.1 }}
            transition={{ type: "spring", stiffness: 300 }}
            className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-brand-yellow/20 text-brand-yellow-dark"
          >
            <Icon size={20} />
          </motion.span>
          <Badge variant="outline" className="transition-colors group-hover:border-brand-yellow-dark">
            {meta}
          </Badge>
        </div>
        ...
        <Link
          to={to}
          className="mt-6 inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#111111] px-5 text-sm font-semibold text-white transition-all duration-300 hover:bg-black hover:gap-3 active:scale-[0.985] will-change-transform"
        >
...
function AttentionRow({
  ...
  return (
    ...
    >
      <Link
        to={to}
        className="flex items-center justify-between gap-4 rounded-xl border border-gray-100 bg-white p-4 transition-all hover:border-brand-yellow/50 hover:shadow-sm"
      >
        <div>
        ...
        <motion.span
          ...
          className={cn(
            "grid h-10 min-w-10 shrink-0 place-items-center rounded-full border px-2 text-sm font-bold",
            cls
          )}
        >
          {value}
...
function SummaryLine({ label, value }: { label: string; value: string | number }) {
  return (
    <motion.div
      whileHover={{ x: 2 }}
      className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2.5 transition-colors hover:bg-gray-100"
    >
